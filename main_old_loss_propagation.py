...
import logging
import csv
from latten_bucket_coverage import get_buckets, jensen_shannon_loss, lsh_diversity_loss, wasserstein_loss, sliced_wasserstein_loss
from scipy.stats import wasserstein_distance

task_index = int(sys.argv[1]) if len(sys.argv) > 1 else 0

logger = logging.getLogger(__name__)
logging.basicConfig(filename=f'task_{task_index}/output.log', encoding='utf-8', level=logging.DEBUG)
...
for epoch in range(EPOCHS):
    ...
    for i, (cond, real_images) in enumerate(train_loader):
        ...
        latten_dist = sliced_wasserstein_loss(latents_fake, latents_real)
...
with open(f'task_{task_index}/results.csv', 'w') as file:
    ...
    for ld, loss, wass in zip(latten_distances_thorugh_epochs, losses_thorugh_epochs, wassersteine_dist_list):
